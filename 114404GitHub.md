...
### 新增/修改檔案
   ...
   4. 將本地資料同步至 GitHub `git push -u origin 你的分支`
      ...
      >之後可只輸入 `git push`
   5. 發起合併請求
      > 在 Your branches 的最右邊 `．．．` 裡的 `New pull request`
...
* 分支(Branch)
  ...
  > | 功能 | 指令 |
  > | :-- | :-- |
  > | 建立分支(branch) | `git branch 新分支名稱` |
  > | 建立並切換新分支(branch) | `git checkout -b 新分支名稱` |
  > | 查看目前有哪些分支 & 目前所在分支 | `git branch` |
  > | 切換分支(branch) | `git checkout 你的分支` |
  > | 刪除分支(branch) | `git branch -d 要刪除的分支` |
  >
  >>
  >> GitHub 點選
