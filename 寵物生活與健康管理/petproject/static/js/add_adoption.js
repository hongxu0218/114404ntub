...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Note: vaccine is now handled by multi-select interface in the main template
    // const vaccineSelect = document.getElementById('vaccine_select'); // No longer exists
    const vaccineOtherGroup = document.getElementById('vaccine-other-group'); // Updated ID
    ...
    // ===== 函式: 更新疫苗 (現在由 multi-select 介面處理) =====
    // function updateVaccine() {
    //     // Vaccine selection is now handled by the multi-select interface in the main template
    // }
...
function updateSpecies() {
    ...
    // Vaccine options are now handled by the multi-select interface
    ...
    // 呼叫更新函式確保「其他」文字框顯示
    updateBreed();
    // updateVaccine(); // No longer needed - handled by multi-select
}
    ...
    // vaccineSelect event is handled by multi-select interface

    // ===== 提交前同步 hidden =====
    if (form) {
        form.addEventListener('submit', function (e) {
            if (speciesSelect && speciesSelect.value === '其他' && speciesOtherInput) {
                hiddenSpecies.value = speciesOtherInput.value.trim();
            }
            if (breedSelect && breedSelect.value === '其他' && breedOtherInput) {
                hiddenBreed.value = breedOtherInput.value.trim();
            }
            // Vaccine submission is handled by multi-select interface

            // 名字重複檢查
            if (nameInput && otherPetNames) {
...
// Note: These are now handled by the multi-select interface in the main template
// function setupOtherToggle(selectId, groupId, inputId) {
//   const selectEl = document.getElementById(selectId);
//   const groupEl = document.getElementById(groupId);
//   const inputEl = document.getElementById(inputId);

//   if (!selectEl || !groupEl || !inputEl) return; // Safety check
...
// These are now handled by the multi-select interface:
