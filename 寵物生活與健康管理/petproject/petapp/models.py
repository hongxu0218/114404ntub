...
class VetAppointment(models.Model):
    ...
    # ✓ 新增文字欄位替代
    clinic_name = models.CharField(max_length=100, verbose_name="診所名稱")
    clinic_address = models.CharField(max_length=200, blank=True, verbose_name="診所地址")
    clinic_phone = models.CharField(max_length=20, blank=True, verbose_name="診所電話")
    doctor_name = models.CharField(max_length=50, blank=True, verbose_name="醫師姓名")

    # 預約時間（從 slot 改為直接欄位）
    appointment_date = models.DateField(verbose_name="就診日期")
    appointment_time = models.TimeField(verbose_name="就診時間")

    # 預約資訊
    reason = models.TextField(verbose_name="就診原因")
    notes = models.TextField(verbose_name="備註", blank=True)
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='scheduled')

    # Google Calendar 整合（保留）
    google_calendar_event_id = models.CharField(max_length=255, blank=True, null=True, verbose_name='Google Calendar Event ID')
    ...
    class Meta:
        verbose_name = '就診記錄'
        verbose_name_plural = '就診記錄'
        ordering = ['-appointment_date', '-appointment_time']

    def __str__(self):
        return f"{self.pet.name} - {self.clinic_name} ({self.appointment_date})"
...
class Profile(models.Model):
    ...
    # Google Calendar 整合
    google_calendar_token = models.TextField(blank=True, null=True, verbose_name='Google Calendar Token')
    google_calendar_enabled = models.BooleanField(default=False, verbose_name='啟用 Google Calendar 同步')
...
class Pet(models.Model):
    ...
    chip = models.CharField(max_length=15, blank=True, null=True, unique=True, verbose_name='晶片號碼')
...
class Report(models.Model):
    ...
    pet = models.ForeignKey('Pet', on_delete=models.CASCADE, related_name='reports', verbose_name='寵物')
    uploaded_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, verbose_name='上傳者')

    # 報告基本資訊
    title = models.CharField(max_length=200, default='醫療檢驗報告', verbose_name='報告標題')
    report_type = models.CharField(max_length=20, choices=REPORT_TYPE_CHOICES, default='other', verbose_name='報告類型')
    report_date = models.DateField(default=date.today, verbose_name='檢驗日期')
    clinic_name = models.CharField(max_length=200, default='未知診所', verbose_name='醫院/診所名稱')

    # PDF檔案
    pdf = models.FileField(upload_to='reports/', verbose_name='PDF檔案')

    # 系統時間戳記
    date_uploaded = models.DateTimeField(auto_now_add=True, verbose_name='上傳日期')
    updated_at = models.DateTimeField(auto_now=True, verbose_name='最後更新日期')

    class Meta:
        verbose_name = '檢驗報告'
        verbose_name_plural = '檢驗報告'
        ordering = ['-report_date', '-date_uploaded']

    def __str__(self):
        return f'{self.pet.name} - {self.title} ({self.get_report_type_display()})'
...
class MedicalRecord(models.Model):
    ...
    visit_date = models.DateField(verbose_name="看診日期")
...
class AdoptionApplication(models.Model):
    ...
    # 基本關聯
    pet = models.ForeignKey(AdoptionPet, on_delete=models.CASCADE, related_name='applications', verbose_name='領養寵物')
    applicant = models.ForeignKey(User, on_delete=models.CASCADE, related_name='adoption_applications', verbose_name='申請人')
    owner = models.ForeignKey(User, on_delete=models.CASCADE, related_name='pet_applications', verbose_name='原飼主')
    ...
    contact_line = models.CharField(max_length=50, blank=True, verbose_name='LINE ID')
    contact_email = models.EmailField(verbose_name='聯絡信箱')

    # 申請者背景資料
    age = models.IntegerField(verbose_name='年齡')
    occupation = models.CharField(max_length=50, verbose_name='職業')
    residence_type = models.CharField(max_length=20, choices=[
        ('house', '獨棟房屋'),
        ('apartment', '公寓大樓'),
        ('townhouse', '透天厝'),
        ('rental', '租屋'),
    ], verbose_name='居住類型')
    residence_ownership = models.CharField(max_length=20, choices=[
        ('own', '自有'),
        ('family', '家人所有'),
        ('rent', '租賃'),
    ], verbose_name='房屋所有權')
    has_yard = models.BooleanField(default=False, verbose_name='是否有庭院')
    yard_size = models.CharField(max_length=50, blank=True, verbose_name='庭院大小')

    # 家庭狀況
    family_members = models.IntegerField(verbose_name='家庭成員人數')
    family_agree = models.BooleanField(default=False, verbose_name='家人是否同意領養')
    has_children = models.BooleanField(default=False, verbose_name='家中是否有小孩')
    children_age = models.CharField(max_length=50, blank=True, verbose_name='小孩年齡')

    # 寵物飼養經驗
    has_pet_experience = models.BooleanField(default=False, verbose_name='是否有飼養經驗')
    pet_experience_detail = models.TextField(blank=True, verbose_name='飼養經驗詳情')
    current_pets = models.TextField(blank=True, verbose_name='目前飼養的寵物')
    pets_count = models.IntegerField(default=0, verbose_name='目前飼養寵物數量')
    ...
    can_afford_medical = models.BooleanField(default=False, verbose_name='有能力負擔醫療費用')

    # 領養動機與承諾
    adoption_reason = models.TextField(verbose_name='領養原因')
    how_to_care = models.TextField(verbose_name='如何照顧寵物的計畫')
    daily_time_for_pet = models.CharField(max_length=50, verbose_name='每日陪伴時間')
    emergency_plan = models.TextField(blank=True, verbose_name='緊急狀況處理計畫')

    # 其他資訊
    additional_message = models.TextField(blank=True, verbose_name='其他想說的話')
    is_willing_home_visit = models.BooleanField(default=False, verbose_name='願意接受家訪')

    # 審核相關
    reviewed_at = models.DateTimeField(null=True, blank=True, verbose_name='審核時間')
    reject_reason = models.TextField(blank=True, verbose_name='拒絕原因')
    owner_notes = models.TextField(blank=True, verbose_name='飼主備註')
    ...
    # 已讀狀態
    owner_has_seen = models.BooleanField(default=False, verbose_name='原飼主已讀')
    applicant_has_seen_response = models.BooleanField(default=False, verbose_name='申請人已讀回覆')

    class Meta:
        verbose_name = '領養申請'
        verbose_name_plural = '領養申請'
        ordering = ['-created_at']

    def __str__(self):
        return f'{self.applicant.username} 申請領養 {self.pet.name} ({self.get_status_display()})'

    @property
    def can_be_accepted(self):
        """檢查是否可以接受申請"""
        return self.status in ['pending', 'reviewing'] and not self.pet.is_adopted
...
class ApplicationMessage(models.Model):
    application = models.ForeignKey(AdoptionApplication, on_delete=models.CASCADE, related_name='messages', verbose_name='領養申請')
    sender = models.ForeignKey(User, on_delete=models.CASCADE, related_name='sent_messages', verbose_name='寄件人')
    recipient = models.ForeignKey(User, on_delete=models.CASCADE, related_name='received_messages', verbose_name='收件人')
    message = models.TextField(verbose_name='訊息')
    created_at = models.DateTimeField(auto_now_add=True, verbose_name='訊息時間')

    def __str__(self):
        return f'Message from {self.sender.username} to {self.recipient.username} for application {self.application.id}'
...
class ApplicantProfile(models.Model):
    """申請人資料設定 - 讓飼主預先設定申請資料，避免重複填寫"""

    user = models.OneToOneField(User, on_delete=models.CASCADE, related_name='applicant_profile', verbose_name='使用者')
    ...
    occupation = models.CharField(max_length=50, blank=True, verbose_name='職業')
    residence_type = models.CharField(max_length=20, blank=True, choices=[
        ('house', '獨棟房屋'),
        ('apartment', '公寓大樓'),
        ('townhouse', '透天厝'),
        ('rental', '租屋'),
    ], verbose_name='居住類型')
    residence_ownership = models.CharField(max_length=20, blank=True, choices=[
        ('own', '自有'),
        ('family', '家人所有'),
        ('rent', '租賃'),
    ], verbose_name='房屋所有權')
    ...
    # 家庭狀況
    family_members = models.IntegerField(null=True, blank=True, verbose_name='家庭成員人數')
    family_agree = models.BooleanField(default=False, verbose_name='家人是否同意領養')
    has_children = models.BooleanField(default=False, verbose_name='家中是否有小孩')
    children_age = models.CharField(max_length=50, blank=True, verbose_name='小孩年齡')
    ...
    # 經濟狀況
    monthly_income_range = models.CharField(max_length=20, blank=True, choices=[
        ('below_30k', '3萬以下'),
        ('30k_50k', '3-5萬'),
        ('50k_70k', '5-7萬'),
        ('70k_100k', '7-10萬'),
    ...
    # 其他預設資訊
    daily_time_for_pet = models.CharField(max_length=50, blank=True, verbose_name='每日可陪伴時間')
    emergency_plan = models.TextField(blank=True, verbose_name='緊急狀況處理計畫')
    is_willing_home_visit = models.BooleanField(default=False, verbose_name='願意接受家訪')
    ...
    class Meta:
        verbose_name = '申請人資料設定'
        verbose_name_plural = '申請人資料設定'

    def __str__(self):
        return f'{self.user.username} 的申請資料設定'
...
class Notification(models.Model):
    ...
    NOTIFICATION_TYPES = [
        ...
        ('adoption_application_new', '新的領養申請'),
        ('adoption_application_accepted', '領養申請已接受'),
        ('adoption_application_rejected', '領養申請已拒絕'),
        ('adoption_application_cancelled', '領養申請已取消'),
        ('system', '系統通知'),
    ]
