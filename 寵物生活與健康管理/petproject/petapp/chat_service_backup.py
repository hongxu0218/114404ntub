...
try:
    ...
    # 使用384維的模型以匹配現有資料庫
    _embedder = SentenceTransformer("sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2")  # 自動配置  # 自動配置  # 自動配置  # 自動配置  # 自動配置  # 自動配置  # 384維
except Exception:
    try:
        # 備用選項：中文模型（如果維度匹配）
...
def _get_cached_client():
    ...
    if _client_cache is None or _collection_cache is None:
        ...
        try:
            ...
            _collection_cache = _client_cache.get_collection(COLLECTION_NAME)  # 直接獲取已存在的集合
        except Exception as e:
...
def safe_retrieve(query: str, top_k: int = TOP_K):
    ...
    try:
        ...
        print(f"[DEBUG] 檢索結果: {len(res.get('documents', [[]])[0])} 筆")
        ...
        # 使用更寬鬆的門檻以更快找到結果
        threshold = 0.2  # 降低門檻以匹配實際相似度
        pairs = []

        for d, m, dist in zip(docs, metas, dists):
            try:
                # 修正相似度計算 - 使用絕對值並確保在合理範圍內
                distance = abs(float(dist))  # 確保距離為正數
                if distance > 2.0:  # 餘弦距離通常在 0-2 之間
                    distance = 2.0
                sim = max(0.0, 1.0 - distance)  # 確保相似度不為負數
                print(f"[DEBUG] 距離: {dist:.3f} -> 相似度: {sim:.3f}, 門檻: {threshold}")
                if sim >= threshold:
                    pairs.append((d, m or {}, sim))
                    print(f"[DEBUG] 加入結果 (相似度: {sim:.3f})")
                else:
                    print(f"[DEBUG] 低於門檻 (相似度: {sim:.3f})")
            except Exception as e:
                print(f"[DEBUG] 處理結果錯誤: {e}")
                continue
...
def api_kb_status(request):
    ...
    try:
        ...
        try:
            col = client.get_collection(COLLECTION_NAME)  # 直接獲取已存在的集合
        except Exception:
            col = client.get_or_create_collection(COLLECTION_NAME)  # 如果不存在才創建

        # 獲取實際文檔總數
        try:
            info["count"] = col.count()
        except Exception as e:
            info["error"] = f"count error: {e}"
    except Exception as e:
        info["error"] = str(e)
